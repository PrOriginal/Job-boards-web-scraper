...
def get_html_justjoin(url):
    ...
    driver.get(url)
    # Zooms out page to get all divs
...
def get_content_pracuj(soup):
    ...
    for url in job_urls:
        ...
        expected_tech.append([e.get_text() for e in soup.find_all("li", class_= "offer-viewjJiyAa offer-vieweKR6vg")])
        
        # Optional technologies have the same id as a required ones. To avoid duplication if statment is added
        optional_tech.append([e.get_text() for e in soup.find_all("p", class_= "offer-viewU0gxPf") if e.get_text() not in expected_tech[i]])
        i+=1
        
        salary = soup.find("strong", class_= "offer-viewLdvtPw")
        if salary != None:
            salaries.append(unicodedata.normalize('NFKC',salary.get_text()))
        else:
...
def get_content_justjoin(soup):
    ...
    for url in justjoin_urls:
        ...
        salaries.append([unicodedata.normalize('NFKD', e.get_text()) for e in soup.find_all("span", class_="css-a2pcn2")])
...
print(len(job_urls))
print(len(salaries))
print(len(expected_tech))
print(len(optional_tech))
